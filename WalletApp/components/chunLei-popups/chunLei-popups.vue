<template>
	<view class="mask" :class="!show?'':'mask-show'" :style="{backgroundColor:show?maskBg:'rgba(0,0,0,0)'}" @tap="tapMask">
		<view class="popups" :class="[theme]"
			...
			<view v-for="(item,index) in popData" :key="index" @tap.stop="tapItem(index,item)" 
			 :style="{color:currect==index?'rgba(17, 196, 209, 1)':''}"
...
<script>
	export default{
		...
		mounted() {
			this.popupsPosition()
			this.currect=uni.getStorageSync('locaIndex')
		},
		methods:{
			tapMask(){
			...
			tapItem(index,item){
				uni.setStorageSync('locaIndex',index)
				this.currect=uni.getStorageSync('locaIndex')
