	...
	<view class="content">
		...
		<view class="collection">
			<view class="collection-adress">
				<view class="collection-adr">{{$t('Assets.gatheringAddress')}}</view>
				<input style="margin-bottom: 30rpx;" type="text" :placeholder="$t('Assets.placeholder')"
					placeholder-class="color: #afafaf;font-size: 28rpx;font-weight: 400;">
		...
		<view class="money">
			<view class="money-transfer">
				<view class="transfer-amount">{{$t('Assets.transferAmount')}}</view>
				<view class="transfer-num">
					<input type="text" :placeholder="$t('Assets.theNumber')"
						placeholder-class="color: #999999;font-size: 32rpx;">
					<text>{{$t('Assets.complete')}}</text>
				</view>
			</view>
			<view class="balance">
				<view class="balance-amount">{{$t('Assets.balance')}}</view>
		...
		<view class="absenteeism">
			<view class="absenteeism-title">{{$t('Assets.workmanship')}}</view>
			<view class="absenteeism-con">
				<view v-for="(item, index) in absenteeismList" :key="index" @click="rechargeChange(index)"
					:class="current == index ? 'scoll_content_item_active': ''" class="absenteeism-type">
					<view class="absenteeism-type-speed">{{item.speed}}</view>
					<view class="absenteeism-type-money">{{item.money}}BNB</view>
			...
			<view class="info">
				<view class="info-title">{{$t('Assets.theChain')}}</view>
				<view class="info-textarea">
					<textarea placeholder-style="color:#999999;
						  font-size: 28rpx;
						  border-radius: 15rpx;
						  background-color: #F3F7F8;
						  padding: 24rpx;" :placeholder="$t('Assets.textPlaceholder')" class="textarea" />
				</view>
			</view>
		</view>
		<!-- 礦工費 -->
		...
		<u-popup :show="show" @close="close" @open="open" round="32rpx">
			<view class="center-popup">
				<view class="t-popup">
					<text>{{$t('Assets.particulars')}}</text>
		...
		<!-- 输入密码 -->
		<u-modal :show="modalShow" :title="$t('CrossChain.title8')" :confirmText="$t('CrossChain.butName')"
			:zoom="false" :showCancelButton="true" confirmColor="#11C4D1" @confirm="confirmClick" @cancel="cancelClick">
			<view class="slot-content" style="width: 517rpx;height: 75rpx;">
				<u-input :placeholder="$t('CrossChain.inputPwd')" border="surround" :password="iconShow"
					v-model="content" @change="change">
	...
	export default {
		...
		onLoad() {
			uni.setNavigationBarTitle({
				title: this.$t('Assets.transferNav')
			});
			this.absenteeismList = [{
					speed: this.$t('Assets.slow'),
					money: 0.000105,
					exchnage: 0.0304,
					time: 3
				},
				{
					speed: this.$t('Assets.recommend'),
					money: 0.000105,
					exchnage: 0.0304,
					time: 3
				},
				{
					speed: this.$t('Assets.fast'),
					money: 0.000105,
					exchnage: 0.0304,
					time: 3
				}
	...
	.content {
		...
		.absenteeism {
			...
			.absenteeism-con {
				...
				.scoll_content_item_active {
					background-color: rgba(17, 196, 209, 0.0600);
					border: 2rpx solid rgba(17, 196, 209, 1);
				}
